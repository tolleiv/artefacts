...
exports.forwardResultField = function(req, res) {
    if (typeof req.data.result[req.params.field] == 'string') {
        res.json(200, req.data.result[req.params.field])
    } else {
        res.json(409, {message: 'field unknown'})
    }
};
...
exports.emitResult = function (req, res, next) {
    if (req.data.action) {
        ...
        this.get("events")(JSON.stringify({
            event: 'object-' + req.data.action,
            data: {type: req.data.type, id: req.data.result.id || req.params.id}
        }));
    }
    next();
};
