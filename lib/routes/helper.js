...
exports.getArtefact = getArtefact = function(app, id, callback) {
    var m = app.get('model').Artefact.using(app.get('connection'))
    m.where('id = ?', id).first(callback)
};
exports.getState = getState = function(app, id, callback) {
    var m = app.get('model').State.using(app.get('connection'))
    m.where('id = ?', id).first(callback)
};

exports.resolveRelatedObjects = function(app, data, callbackOk, callbackNok) {
    ...
    if (data['artefact_id']) list.artefact = getArtefact.bind(this, app, data['artefact_id']);
    if (data['state_id']) list.state = getState.bind(this, app, data['state_id']);
